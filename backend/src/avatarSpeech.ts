...
export async function generateAvatarSpeech(
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      // award/strip pipeline so we don't accidentally lose award mentions.
      const scrub = (text: string): string => {
        ...
        const LONGTERM: RegExp[] = [
          ...
          /отсутству(ют|ет)\s+(данные\s+о\s+)?долгосрочн(ых|ом|ой)\s+влияни/giu,
          /не\s+(показано|приведено|представлено)\s+(данных\s+о\s+)?долгосрочн(ых|ом|ой)/giu,
          /долгосрочн(ое|ого)\s+(бизнес-)?(результат|влияние)\s+(не|отсутствует)/giu,
        ];
        let out = text;
        for (const re of VAGUE) out = out.replace(re, "");
        for (const re of LONGTERM) out = out.replace(re, "не хватает измеримых результатов в текущем периоде кампании");
        return out.replace(/\s+/g, " ").replace(/\s+([.,;:!?])/g, "$1").trim();
      };
      parsed.one_paragraph_verdict = scrub(parsed.one_paragraph_verdict);
      parsed.short = scrub(parsed.short);
      parsed.long = scrub(parsed.long);
      parsed.sections.hook = scrub(parsed.sections.hook);
      parsed.sections.verdict = scrub(parsed.sections.verdict);
      parsed.sections.steelman = scrub(parsed.sections.steelman);
      parsed.sections.fatal_flaw = scrub(parsed.sections.fatal_flaw);
      parsed.sections.close = scrub(parsed.sections.close);
