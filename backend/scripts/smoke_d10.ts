...
async function main() {
  ...
  const nom = noms.find((n) => n?.code === "D10");
  const p = nom.projects.find((x: any) => x.project_id === "8588");
  ...
  const bundle: CaseBundle = {
    metadata: { case_id: `smoke-${p.project_id}`, year: "2026", nomination_id: "D10", block_id: "53", project_id: p.project_id, project_name: p.project_name },
    text_fields,
    extracted_text: [
  ...
  console.log(`scoring D10 / 8588 — ${p.project_name}`);
  const t0 = Date.now();
  const out = await runAnalysis(bundle);
  console.log(`done in ${((Date.now() - t0) / 1000).toFixed(1)}s — block_score=${out.l2.block_score} total=${out.l2.total_score} award=${out.l2.award_level} caps=${out.l2.caps_applied?.length ?? 0}`);
